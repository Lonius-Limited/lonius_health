...
def get_draft_invoice_total(invoice_doc):
	# SOMEHOW INVOICE TOTALS IS NOT REFLECTED IMMEDIATELY IN THE SERVER FOR DRAFTS.
...
def validate_payment(doc, handler=None, ignore_zero_balance=False):
	...
	if (flt(this_invoice_total) > flt(balance)):
		needed_to_proceed = fmt_money(this_invoice_total - balance)
		balance = fmt_money(balance - (this_invoice_total -
									   (this_invoice_total - balance)))
		currency = get_defaults().get('currency')

		# INSERT DRAFT PAYMENT ENTRY
		amount = flt(this_invoice_total) - flt(balance)
...
def validate_insurance_limit(doc):  # SALES INVOICE
	...
	if limit < doc.get('total'):
		...
		for idx in row_idxs:
			actual = idx + 1
			row = list(filter(lambda x: x.get('idx') == actual, doc.get('items')))[0]  # doc.get('items')[idx]
			# toadd = []
			cummulative += row.get('amount')
			row_count += 1
			items.append(dict(item_code=row.get('item_code'), qty=row.get('qty')))
			items_to_reverse.append(dict(item_code=row.get('item_code'), qty=-row.get('qty'), rate= row.get('rate'), amount= row.get('amount')))
			if cummulative > difference:
				break
		toprint = [x.get('item_code') for x in items]
		for item_reverse in items_to_reverse: doc.append('items', item_reverse)
		doc.run_method('set_missing_values')
		...
		if items:
			...
			frappe.msgprint("A cash invoice has been posted instead for the excess bill.")
			# REMOVE THE SAME ITEMS FROM THE INSRUANCE INVOICE. IF EVERYTHING IS BEING REMOVED, DELETE THE OLD INVOICE
			# if len(doc.get('items')) == row_count:
			# 	doc.delete()
			# 	return
			# the_index = 0
			# to_remove = []
			...
			# 	the_index += 1
			# 	if len(doc.get('items')) - the_index < row_count:
